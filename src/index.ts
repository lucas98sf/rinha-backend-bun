    ...
    async ({ query, set }) => {
      try {
        ...
        const pessoasFound = await db
          ...
          .where(
            sql.raw(`
              to_tsvector('english', apelido) @@ plainto_tsquery('"${query.t}":*')
              OR to_tsvector('english', nome) @@ plainto_tsquery('"${query.t}":*')
              OR to_tsvector('english', stack) @@ plainto_tsquery('"${query.t}":*')
            `),
          )
          .limit(50)
          .execute();
